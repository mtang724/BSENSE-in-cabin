...
def custom_collate_fn(batch):
    ...
    doppler_data = torch.cat(doppler_data_tun, dim=0)
...
for exp_id in range(0, 5):
    ...
    model_path = os.path.join('./model_weights/incar_weights', 'whole_exp_{}_front_incar_best_model_row{}.pth'.format(exp_id, row))
    # Load the model
    ...
    with torch.no_grad():
        for i, (batch_data, batch_phase_data, batch_labels, batch_res_labels, data_name) in enumerate(test_loader):
            ...
            outputs1_sum = smooth_signal(outputs1_sum, 3)     
            ...
            for i, seat in enumerate(seat_loss1):
                ...
                if loss != 0:
                    bpm_errors.append(loss.cpu())
                # print(f'{seat} Test Loss{loss}')
            
print("Avg BPM error:", np.mean(bpm_errors))
true_labels = np.concatenate(true_labels, axis=0)
pred_labels = np.concatenate(pred_labels, axis=0)
acc_score_seat = accuracy_score(true_labels, pred_labels)
print(f"Detection Rate for 5 Experiments: {acc_score_seat}")
