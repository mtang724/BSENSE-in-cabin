...
indoor_file_list_data_path = "./file_list/experiment_list/front_babydoll+adultsfile_list.txt"
...
def custom_collate_fn(batch):
    ...
    # doppler_data_tun = []
    # for dopp in doppler_data:
    #     dopp = dopp[:, :, :9, :]
    #     doppler_data_tun.append(dopp)
...
for exp_id in range(0, 5):
    ...
    train_filenames = load_filenames_from_txt(f'./file_list/incar_row2_file_list/train_filenames_fold_{exp_id}.txt')
    val_filenames = load_filenames_from_txt(f'./file_list/incar_row2_file_list/val_filenames_fold_{exp_id}.txt')
    test_filenames = load_filenames_from_txt(f'./file_list/incar_row2_file_list/test_filenames_fold_{exp_id}.txt')
    ...
    doppler_model = ResNetDoppler(BasicBlock, [2, 2, 2], num_classes=2).to(device)
    aoa_aod_model = AoA_AoD_Model().to(device)
    
    # aoa_aod_model = torch.load(os.path.join('/u/mt55/BSENSE/model_training/train_and_inference/model_weights/sep_weights/0928aoaaod_exp_0_front_indoor_best_model_row2.pth')).to(device)
    # doppler_model = torch.load(os.path.join('/u/mt55/BSENSE/model_training/train_and_inference/model_weights/sep_weights/0928resdop_exp_0_front_indoor_best_model_row2.pth')).to(device)
    
    feature_dim = 128
    seat_output_dim = 2
    resp_output_dim = 2
    model = CombinedModelOneDecoder(aoa_aod_model, doppler_model, feature_dim, seat_output_dim, resp_output_dim).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        if best_val_loss > avg_val_loss:
            ...
            torch.save(model, os.path.join('./model_weights/incar_weights', 'whole_exp_{}_front_incar_best_model_row{}.pth'.format(exp_id, row)))
        # evaluation
        model.eval()  # set the model to evaluation mode
        total_loss = 0
        true_labels = []
